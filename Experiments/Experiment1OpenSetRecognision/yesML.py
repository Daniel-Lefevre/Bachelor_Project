...
class ImageProcessingML:
    ...
    def plot_3d(self):
        ...
        ax.set_ylabel("PC2")
        ax.set_zlabel("PC3")
        ax.set_title("2D LDA of Image Features")
        plt.show()

    def plot_centroid_distance_heatmap(self):
        print("Plotting Centroid Euclidean Distances...")
        num_labels = len(self.labels)
        dist_matrix = np.zeros((num_labels, num_labels))

        # Calculate the pure Euclidean distance from Centroid i to Centroid j
        ...
        cax = ax.imshow(dist_matrix, cmap="plasma")
        ...
        for i in range(num_labels):
            for j in range(num_labels):
                color = "white" if dist_matrix[i, j] < np.max(dist_matrix) / 1.5 else "black"
                ax.text(j, i, f"{dist_matrix[i, j]:,.1f}", ha="center", va="center", color=color, fontsize=8)

        ax.set_title("Euclidean Distance Between Centroids\n(Row = Point, Column = Target Cluster)")
        fig.tight_layout()
        plt.show()
...
if __name__ == "__main__":
    ...
    # image_processor.plot_centroid_distance_heatmap()
    ...
    print(f"Time per image classification in validation: {(total_validation_time / 62) * 10 ** (-6)} ms")
